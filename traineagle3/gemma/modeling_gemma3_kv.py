# Source: https://github.com/huggingface/transformers/blob/v4.53-release/src/transformers/models/gemma3/modeling_gemma3.py
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if past_key_value is not None:
            ...
            # [MODIFIED] Using KVCache mechanism for pre-allocated GPU memory optimisation.
            #            We deliberately stop propagating the cache object any further
            #            (Gemma's `DynamicCache` already owns the state), so downstream
            #            tensors are not needlessly serialized.
            past_key_value = None
...
class Gemma3TextModel(Gemma3PreTrainedModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        # [MODIFIED] `past_key_values` is a KVCache instance
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            ...
            causal_mask_mapping = {
                ...
                "sliding_attention": create_sliding_window_causal_mask(**mask_kwargs).to(torch.float32),
            }
            # # Create the masks
            # causal_mask_mapping = {
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict):
            ...
            if token_type_ids is not None and inputs_embeds.shape[1] != 1:
                ...
                mask_kwargs["or_mask_function"] = token_type_ids_mask_function(
                    token_type_ids.to(
                        cache_position.device), self.config.mm_tokens_per_image
                )
        ...
        if labels is not None:
            ...
            if attention_mask is not None:
                ...
                shift_attention_mask = attention_mask[:, -
                                                      shift_logits.shape[1]:].to(logits.device)
    ...
    def create_masks_for_generate(
        ...
        **kwargs,
        ...
        if token_type_ids is not None and input_embeds.shape[1] != 1:
            ...
            mask_kwargs["or_mask_function"] = token_type_ids_mask_function(
                token_type_ids.to(
                    cache_position.device), config.mm_tokens_per_image
            )
