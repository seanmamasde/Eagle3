...
from modeling_gemma3_kv import Gemma3ForCausalLM
...
class Gemma3RotaryEmbedding(torch.nn.Module):
...
class Gemma3Attention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.q_proj = nn.Linear(self.hidden_size * 2,
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            ...
            if scaling_type == "linear":
                self.rotary_emb = Gemma3LinearScalingRotaryEmbedding(
    ...
    def forward(
        ...
        attn_weights = torch.matmul(
            query_states, k0.transpose(2, 3)) / math.sqrt(self.head_dim)
        lck = len(cache_k)
    ...
    def forward(
        ...
        attention_mask = self._prepare_decoder_attention_mask(
            attention_mask, (batch_size,
                             seq_length), hidden_states, past_key_values_length
        )
