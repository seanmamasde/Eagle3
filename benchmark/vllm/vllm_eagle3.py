...
EAGLE_MODEL_DIR = Path("/home/seanma0627/src/eagle3/weights/taide-eagle3-vllm")
PROMPT = "怎麼學習一個新的語言？"
ROUNDS = 10
TRIM_RATIO = 0.2
DTYPE = torch.float16
...
def _build_llm(enforce_eager: bool,
    ...
    return LLM(
        ...
        speculative_config={
            "method": "eagle3",
            "model": str(EAGLE_MODEL_DIR),
            "draft_tensor_parallel_size": TP_SIZE,
            "num_speculative_tokens": SPEC_TOKEN,
            # "dtype": DTYPE,
        },
    )
...
def bench(enforce_eager: bool):
    eagle = _build_llm(enforce_eager, speculative=True)
    spec_lat = _run_rounds(eagle)
    spec_avg = _trimmed_mean(spec_lat)
    base_tps = TOTAL_TOKENS / spec_avg
    print(f"EAGLE3 latency  : {spec_avg:.3f}s  (trimmed mean over {ROUNDS})")
